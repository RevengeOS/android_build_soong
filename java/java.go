...
func (j *Module) deps(ctx android.BottomUpMutatorContext) {
	if ctx.Device() {
		...
		if ctx.ModuleName() == "framework" || ctx.ModuleName() == "framework-annotation-proc" {
			ctx.AddDependency(ctx.Module(), revengeosResTag, "com.revengeos.platform-res")
		}
		if ctx.ModuleName() == "com.revengeos.platform-res" {
			ctx.AddDependency(ctx.Module(), frameworkResTag, "framework-res")
		}
		if ctx.ModuleName() == "com.revengeos.platform" ||
			ctx.ModuleName() == "com.revengeos.platform.internal" ||
			ctx.ModuleName() == "com.revengeos.platform.sdk" {
			ctx.AddDependency(ctx.Module(), revengeosResTag, "com.revengeos.platform-res")
		}
	}
...
func (j *Module) collectDeps(ctx android.ModuleContext) deps {
	...
	ctx.VisitDirectDeps(func(module android.Module) {
		...
		case Dependency:
			...
			case revengeosResTag:
				if ctx.ModuleName() == "com.revengeos.platform" ||
					ctx.ModuleName() == "com.revengeos.platform.internal" ||
					ctx.ModuleName() == "com.revengeos.platform.sdk" {
					// com.revengeos.platform.jar has a one-off dependency on the R.java and Manifest.java files
					// generated by com.revengeos.platform-res.apk
